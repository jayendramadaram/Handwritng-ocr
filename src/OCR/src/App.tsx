...
function App() {
  ...
  const handleSubmit = (event: any) => {
    if (!imgarr[0]) alert("No Images Selected")
    event.preventDefault();
    const formData = new FormData();
    formData.append('image', imgarr[0]);
    ...
    toast.promise(
      fetch('http://127.0.0.1:5000/upload_image', {
        ...
        .then((data) => {
          console.log(data);
          alert(`Probabilty is ${data.probabity} And word is \"${data.word}\"`)
          setimgarr([])
        }),
      {
        loading: 'Saving...',
          ...
          <article aria-label="File Upload Modal" className="relative h-full flex flex-col bg-white shadow-xl rounded-md">
            ...
            <section className="h-full overflow-auto p-8 w-full flex flex-col">
              <header className="border-dashed border-2 border-gray-400 py-12 flex flex-col justify-center items-center">
                ...
                <input id="hidden-input" type="file" multiple accept='image/png' className="hidden" onChange={(e) => {
                  if (e.target.files) {
                    let arr: Array<File> = []
                    setimgarr([e.target.files[0]])
                    setState(true)
                  }
                }} />
            ...
            <footer className="flex justify-end px-8 pb-8 pt-4">
              <button id="submit" className="rounded-sm px-3 py-1 bg-blue-700 hover:bg-blue-500 text-white focus:shadow-outline focus:outline-none" onClick={(e) => {
                console.log("chechhh");
                handleSubmit(e)
              }}>
                Upload now
